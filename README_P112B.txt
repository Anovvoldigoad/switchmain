NSC2Switch P112B — exact state125 producer entry provenance (READ ONLY)
...
WHY P112B
P111 proved successful vanilla victim admission is req125 -> req126 -> PlayAction12 before the type10 cinematic manager. Failing Tobi own-UJ victim instead takes state39 -> 121 and never reaches victim req125/126/12. The same native whole function main+0x7EB270 is known to request state125, and later the failing attacker Tobi also reaches it at action708 as cleanup. P112B traces the ENTRY of that whole function so the incoming caller and actor target are proven rather than inferred.
...
TEST
1) Boot and confirm: [NSC:P112B] READY ... probe_ok=1
...
5) Upload the compiled P112B artifact + full Uzuy log.
Optional local analyzer: python3 analyze_p112b_log.py uzuy_log.txt
...
P112B correction: runtime fingerprint at main+0x7DDFD0 now uses exact paired-main word F9400268. P112A R123 aborted probe installation because its runtime signature mistakenly expected F9400008 while the source verifier correctly knew F9400268. Hook target and read-only design are unchanged.
