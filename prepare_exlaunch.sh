...
cp "$ROOT/overlay/source/program/p103_specialcond_map.hpp" "$EXL/source/program/p103_specialcond_map.hpp"
...
s,n4=re.subn(r'(?m)^ELF_EXTRACT\s*:=.*$', 'ELF_EXTRACT := $(PWD)/p64f_final.elf', s, count=1)
if (n1,n2,n3,n4)!=(1,1,1,1): raise SystemExit(f'config patch failed {(n1,n2,n3,n4)}')
p.write_text(s)
PY
echo "[+] P103A SpecialCond factory bridge overlay prepared"
