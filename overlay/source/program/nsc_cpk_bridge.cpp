...
constexpr ptrdiff_t kOugiFinishIndexOffset    = 0x44F264;  // P44A target
...
HOOK_DEFINE_TRAMPOLINE(OugiFinishIndexHook) {
    static void Callback(void* a0, void* a1) {
        uint32_t side = 0xFFFFFFFFu, char_id = 0xFFFFFFFFu;
        ReadActorIdentity(a0, side, char_id);
        const uint32_t n = g_ougi_finish_index_logs.fetch_add(1, std::memory_order_relaxed);
        if (n < 1024) {
            Logging.Log("[NSC:P44A] OUGI_FINISH_INDEX a0=%p a1=%p side=%u char=%u n=%u", a0, a1, side, char_id, n);
        }
        Orig(a0, a1);
    }
};
