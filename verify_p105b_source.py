...
start=cpp.find('// P105B — single-trampoline sibling-controller proof')
assert start>=0
block=cpp[start:]
hs=cpp.find('HOOK_DEFINE_TRAMPOLINE(P105BController4C0Hook)',start)
he=cpp.find('static bool InstallP105BSingleTrampolineControllerTraceInternal()',hs)
assert hs>=0 and he>hs
h=cpp[hs:he]
def chk(name,v): print(name,'PASS' if v else 'FAIL'); assert v,name
def pos(s,n): i=s.find(n); assert i>=0,n; return i
chk('4c0_void_abi','static void Callback(void* actor, uint32_t mode)' in h)
chk('4c0_caller_first',pos(h,'asm volatile("mov %0, x30"') < pos(h,'ReadActorIdentity(actor, side, cid)'))
chk('4c0_orig_paths',len(re.findall(r'^\s*Orig\(actor, mode\);\s*$',h,re.M))==2)
chk('4c0_player_filter','side == 0u' in h)
chk('4c0_pre_before_orig',pos(h,'const P93CoreState pre = ReadP93CoreState(actor);') < h.rfind('Orig(actor, mode);'))
chk('4c0_post_after_orig',h.rfind('Orig(actor, mode);') < pos(h,'const P93CoreState post = ReadP93CoreState(actor);'))
code_only=re.sub(r'//.*?$|/\*.*?\*/|"(?:\\.|[^"\\])*"','',block,flags=re.M|re.S)
checks={
 'entrypoint':'nsc::InstallP105BSingleTrampolineControllerTrace();' in main,
 'decl':'void InstallP105BSingleTrampolineControllerTrace();' in hpp,
 'parent_p96':'InstallP96AActionDescriptorTransitionTrace();' in block,
 'ready':'[NSC:P105B] READY' in block,
 'ctrl4c0_log':'[NSC:P105B] CTRL4C0' in block,
 'offset_4c0':'kP105BController4C0Offset = 0x7DDD94' in block,
 'one_p105b_trampoline':cpp.count('HOOK_DEFINE_TRAMPOLINE(P105BController')==1,
 'no_p105b_inline':'HOOK_DEFINE_INLINE(P105B' not in cpp,
 'no_520_trampoline':'P105BController520Hook' not in cpp and 'P105AController520Hook' not in cpp,
 ...
 'p104_runtime_absent':'[NSC:P104B]' not in cpp,
...
expected={
 ...
 0x7DDDA4:0xA90757F6,0x7DDDA8:0xA9084FF4,0x7DE554:0x97FE218E,
...
def bl_target(off):
 ...
 return off+(imm<<2)
for off,tgt in [(0x7DE554,0x766B8C),(0x7E6EC4,0x766B8C),(0x488B28,0x7E64D4)]:
...
print('push_enabled_workflows',push); assert push==['build-subsdk9-p105b.yml']
print('P105B_DROPIN_SOURCE_VERIFY=PASS')
