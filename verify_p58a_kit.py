...
ck('calls_p58','InstallP58APlayActionCallerTrace();' in main)
ck('playaction_existing_hook','PlayActionProbeHook::InstallAtOffset(kPlayActionProbeOffset);' in cpp)
ck('p58_capture_x30','asm volatile("mov %0, x30"' in cpp)
ck('p58_play_marker','[NSC:P58A] PLAY_CALL' in cpp)
ck('p58_custom_all','const bool p58_log = custom || vanilla_uj_start;' in cpp)
ck('p58_callsite_minus4','caller_off >= 4 ? caller_off - 4 : -1' in cpp)
ck('central_setter_retained','InstallP57CentralSetterTrace();' in cpp)
ck('ready_6','total_trampolines=6 writes=0 custom_all_play_calls=1' in cpp)
...
if failed:
    print('P58A_VERIFY=FAIL',','.join(failed)); sys.exit(1)
print('P58A_VERIFY=PASS')
print('active_trampolines=6')
print('added_over_p57=0')
