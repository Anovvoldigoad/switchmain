...
======================================================================
86. R81 — P80B EXACT CONDITION ROOT RESOLVED
======================================================================
...
1b3b3150c6415e6e89f55812ba0f3b8e1a8814c01ef4b4fa42116e9547ab7350


----------------------------------------------------------------------
86.1 P80B INSTALLATION
----------------------------------------------------------------------
...
----------------------------------------------------------------------
86.2 EXACT NATIVE CONDITION MATCH
----------------------------------------------------------------------

Native Predicate-B query:

main+0x777388(collection, wanted_type=1)
...
SW_MTOB_XH / condition index512 is the exact entry making
Predicate B positive.


----------------------------------------------------------------------
86.3 FULL Tobi-PLAYER QUERY CENSUS
----------------------------------------------------------------------
...
throughout the captured Tobi-player phase, every focused Predicate-B
classification resolves to condition index512.


----------------------------------------------------------------------
86.4 FAILED XXA TIMING
----------------------------------------------------------------------
...
P77 UJ_ACCEPT for Tobi:

0


----------------------------------------------------------------------
86.5 VANILLA POSITIVE CONTROL
----------------------------------------------------------------------
...
P77 native UJ acceptance path remains healthy.


----------------------------------------------------------------------
86.6 CONDITION DATA IS NOT CORRUPTED
----------------------------------------------------------------------

Historical compiler/source audit proves:

Tobi conditionprmManager records remain byte-identical to their source.

Runtime descriptor for SW_MTOB_XH:

(1, 8, 5, 5)

matches the compiled high-bank source data.

Do NOT repair this by mutating field +0x0C.


Vanilla named AWAKE condition records also use field +0x0C = 1.

Therefore:

STRONG STATIC/SOURCE CORRELATION:

Predicate B is recognizing a legitimate awakening/type1-style active
condition, not random descriptor corruption.


----------------------------------------------------------------------
86.7 OUGI-AWAKENING POLICY GAP
----------------------------------------------------------------------

UltimateStormAPI PC source contains an OugiAwakeningParam subsystem.

Source semantics:

call original policy;
read actor char ID;
if char ID is present in ougiAwakening set, allow the policy;
otherwise preserve original result.


Source comments explicitly describe the feature as allowing Ultimate
Jutsu in awakening.


Compiled extra OugiAwakeningParam contains:
...
STRONG SOURCE-DERIVED ROOT HYPOTHESIS:

Switch currently recognizes Tobi's type1/awakening condition correctly,
but the corresponding OugiAwakening compatibility policy is not yet
consumed on the active Switch UJ decision path.


======================================================================
87. R81 NEXT ACTION — P81 OUGI-AWAKENING POLICY BRIDGE
======================================================================

Implement a data-driven OugiAwakening policy experiment.

Requirements:

1. Keep P77/P67/P50 baseline.
2. Do not alter condition index512.
3. Do not alter descriptor field +0x0C.
4. Do not force state87.
5. Do not force action700.
6. Do not special-case char281.
7. Generate extra OugiAwakening membership from the compiled
   ougiAwakeningParam data.
8. Scope behavior to:
      Predicate B main+0x7EACF0
      exact parent return main+0x7EAC60
      semantic Ultimate-Jutsu request active
      actor membership in generated OugiAwakening table.
9. Preserve native Predicate B for every other case.
10. Retain P77 native UJ acceptance probe.

Success criterion:

P81 policy removes only the awakening/type1 veto.

Then native path must produce:

P77 UJ_ACCEPT
requested=700
PlayAction700

without directly forcing any of those values.
...
======================================================================
88. R81.1 — P81A SOURCE / POLICY BRIDGE READY
======================================================================

R81 parent checkpoint:

SHA256:
8c6435913764640eda033c196ca99aaf3092a5e5142654ca5b81e242dd0a6df3


----------------------------------------------------------------------
88.1 OUGI MEMBERSHIP INPUT
----------------------------------------------------------------------

Historical compiled OugiAwakening payload was recovered byte-for-byte
from previously proven audit evidence because the old All4Compiled
runtime-input file was no longer present locally.
...
The gameplay bridge does NOT contain a char281 comparison.

Membership is consumed through generated data:

p81_ougi_awake_ids.hpp

using:

ContainsOugiAwakeningId(char_id)


----------------------------------------------------------------------
88.2 P81A ACTIVE CHAIN
----------------------------------------------------------------------
...
P80B deep-query probe is preserved in source but NOT installed.

P79 probe is NOT installed.

P78 probe is NOT installed.


----------------------------------------------------------------------
88.3 P81A POLICY TARGET
----------------------------------------------------------------------
...
Behavior modification only when exact caller LR is:
...
if exact UJ caller
and semantic selector1 active
and actor char ID belongs to generated OugiAwakening membership
and native_ret != 0:
...
Otherwise:
    ...
    policy_ret = native_ret


----------------------------------------------------------------------
88.4 IMPORTANT NON-FORCING CONTRACT
----------------------------------------------------------------------
...
mutate condition descriptor +0x0C

force Predicate A

force P77 acceptance
...
The native UJ corridor remains responsible for all downstream
admission/state/action decisions.


----------------------------------------------------------------------
88.5 BUILD PIPELINE UPDATE
----------------------------------------------------------------------

prepare_exlaunch_p67a.sh now also copies:

p81_ougi_awake_ids.hpp

into:

exlaunch/source/program/

This is required because nsc_cpk_bridge.cpp now includes the generated
membership header.


======================================================================
89. R81.1 NEXT ACTION — VERIFY / BUILD P81A
======================================================================

Create P81 verifier and workflow.

P81A becomes the only automatic probe/fix-experiment workflow.

Build subsdk9.

Runtime success chain:
...
If POLICY allow=1 occurs but P77 acceptance does not occur, do NOT
force downstream values.

Continue tracing the next native UJ-corridor gate.
...
======================================================================
90. R81.2 — P81A FINAL SOURCE / STATIC VERIFIER PASS
======================================================================

P81A full verifier completed successfully.
...
----------------------------------------------------------------------
90.1 GENERATED OUGI MEMBERSHIP
----------------------------------------------------------------------
...
Gameplay behavior remains data-driven through:

ContainsOugiAwakeningId(char_id)

There is no char281 gameplay branch.


----------------------------------------------------------------------
90.2 SOURCE CONTRACT
----------------------------------------------------------------------

P81_MAIN_CHAIN_VERIFY=PASS

P81_HEADER_VERIFY=PASS

P81_CONSTANT_VERIFY=PASS

P81_POLICY_CALLBACK_VERIFY=PASS

P81_BASELINE_VERIFY=PASS

P81_P80_SOURCE_PRESERVE_VERIFY=PASS

P81_PREPARE_VERIFY=PASS


----------------------------------------------------------------------
90.3 STATIC SWITCH TARGET
----------------------------------------------------------------------

P81_PARENT_STATIC_VERIFY=PASS

P81_UJ_CALLSITE_STATIC_VERIFY=PASS

P81_MAIN_PREREQ_VERIFY=PASS
...
P81_STATIC_VERIFY=PASS

P81_SOURCE_VERIFY=PASS


----------------------------------------------------------------------
90.5 VERIFIER CORRECTION
----------------------------------------------------------------------

prepare_exlaunch_p67a.sh contains exactly one copy command for:

p81_ougi_awake_ids.hpp

The generated header filename naturally appears twice on that one line,
once as source and once as destination.

Verifier now counts copy commands rather than raw filename occurrences.
...
======================================================================
91. R81.2 NEXT ACTION — BUILD P81A
======================================================================

Create P81A workflow from proven P80B build workflow.

P81A must become the only automatic probe/fix-experiment workflow.

Historical P64F through P80B workflows remain manual-only.

Build subsdk9.

Runtime decisive sequence:
...
If the policy allows but P77 native acceptance remains absent, do not
force state/action. Continue from the next native UJ-corridor gate.
...
======================================================================
92. R82 — P81A RUNTIME RESULT / P82A DOWNSTREAM TRACE READY
======================================================================

P81A runtime result:

- OugiAwakening membership recognized.
- semantic selector1 remained active.
- native +0x1288 result = 1.
- P81 policy result = 0.
- allow = 1.
...
P81A successfully clears the previously proven awakened/type1 veto.


However, native downstream behavior remains incomplete.
...
No requested=700 was observed.

Therefore:

CORRECTION:

0x8B2B70 ret=1 is NOT final UJ acceptance.

It is only a positive predicate/helper inside the native UJ corridor.


----------------------------------------------------------------------
92.1 NEW ROOT BOUNDARY
----------------------------------------------------------------------

The remaining failure is downstream of:
...
P82A traces the remaining safe normal-function boundaries without
forcing behavior.
...
0x7D2DE4
exact caller returns:
0x7F46C8
0x7F4780

0x7C6280 native control getter
exact caller returns:
0x7F4790
0x7F47E4
0x7F4814
...
P82A keeps P81A fully active.


----------------------------------------------------------------------
92.3 P82A SAFETY CONTRACT
----------------------------------------------------------------------

P82A is read-only.

All Orig() returns are preserved.

P82A does NOT:

hook the inline F58 BLR
...
P82_HOOK_CONTRACT=PASS

P82_READONLY_CONTRACT=PASS

P82_DOWNSTREAM_CALLSITE_STATIC=PASS
...
P82A_SOURCE_SANITY=PASS


======================================================================
93. R82 NEXT ACTION — VERIFY / BUILD / RUN P82A
======================================================================

Perform strict static verifier against the paired main.

Then build P82A as the only automatic probe workflow.

Runtime target:

One Tobi XXA attempt is sufficient.

Collect full log.

Determine exact downstream sequence after P81 allow=1 and P77 ret=1.

The result must identify which native gate still routes to action445.

Do NOT patch behavior until the P82 runtime sequence proves that gate.
...
======================================================================
94. R82.1 — P82A FULL BUILD PREFLIGHT PASS
======================================================================

P82A verifier:

P82_MAIN_CHAIN_VERIFY=PASS
P82_BASELINE_VERIFY=PASS
P82_P81_OUGI_DATA_VERIFY=PASS ids=281
P82_PREPARE_VERIFY=PASS
P82_CALLBACK_VERIFY=PASS
P82_CONSTANT_VERIFY=PASS

P82_794EC8_STATIC_VERIFY=PASS
P82_64A030_STATIC_VERIFY=PASS
P82_7C5FFC_STATIC_VERIFY=PASS
P82_7D2DE4_STATIC_VERIFY=PASS
P82_7C6280_STATIC_VERIFY=PASS

P82_CORRIDOR_CALLSITE_VERIFY=PASS
P82_MAIN_PREREQ_VERIFY=PASS
P82_HEADER_VERIFY=PASS

P82_NSO_LAYOUT_VERIFY=PASS
text_file_off=0x101
text_mem_off=0x0

P82_STATIC_VERIFY=PASS
P82_SOURCE_VERIFY=PASS


Workflow:

P82_WORKFLOW_REQUIRED=PASS
P82_WORKFLOW_STALE=PASS

Historical P64F through P81A workflows:
...
P82A_BUILD_PREFLIGHT=PASS


======================================================================
95. R82.1 NEXT ACTION — COMMIT / CI / RUNTIME
======================================================================

Commit only intended P82A source, verifier, README, and workflow changes.

Do not stage:
- APPLY_P82A_DOWNSTREAM_CORRIDOR.py
- *.pre_p82a
- emulator logs
- downloaded build artifacts
- temporary recovery files

P82A runtime goal remains:

P81 allow=1
    ->
P77 ret=1
    ->
P82 downstream sequence
    ->
requested445

The exact downstream gate responsible for the remaining 445 route must
be identified before any behavior patch is introduced.
...
======================================================================
96. R83 — P83A SLOT1928 POLICY BRIDGE SOURCE READY
======================================================================

P82 runtime result narrowed the remaining UJ failure to the path after:
...
The deeper P82 probes were not reached.

Path exclusion plus static analysis narrowed the next native decision to
the actor virtual slot +0x1928.
...
Observed custom actor vtable:

RVA 0x201BF58

vtable +0x1928 relocation:
...
Therefore current source target:

main+0x7D34E0


Exact UJ-corridor callsite:
...
P83A calls native first.

Only the exact caller return 0x7F474C is eligible for compatibility
override.

A native zero result becomes one only when:

- actor identity is valid
- semantic Ultimate Jutsu state is active
- actor character ID belongs to generated OugiAwakening membership
- native result is zero
...
- special-case char281
- hook the inline virtual BLR
- force state87
- force action700
- rewrite action445
- mutate condition records
- modify generic control getters
...
P83_MAIN_CHAIN=PASS
P83_BASELINE=PASS
P83_POLICY_CONTRACT=PASS
P83_GENERIC_POLICY=PASS

Paired main SHA256:

1adc4dfe948d616cbb7c6d9b3672842aba8e7d86bf0763e668505dff84234de0

P83_SLOT1928_TARGET_STATIC=PASS
P83_SLOT1928_CALLSITE_STATIC=PASS
...
P83_HEADER=PASS
P83A_SOURCE_SANITY=PASS


======================================================================
97. R83 NEXT ACTION — BUILD / RUN P83A
======================================================================

Build P83A while keeping the complete P82 downstream trace installed.
...
P82 H794EC8 ret=0
    ->
P83 SLOT1928_POLICY native=0 policy=1 allow=1

Then observe whether native execution reaches:

P82 H7D2DE4

P82 CTRL_GET

state87 / requested700 / Play700

No further forced behavior is justified until this runtime result.
...
======================================================================
98. R84 — P83A RUNTIME SUCCESS + INPUT-INTENT REGRESSION
======================================================================
...
4f38b6fd5aabfb416cd4d0f31dc0bfd14f48a0ce100c42af2bbe150e8afbd02c


----------------------------------------------------------------------
98.1 P83A ARTIFACT IDENTITY
----------------------------------------------------------------------

Artifact:

NSC-P83A-slot1928-ougi-policy-bridge.zip

Artifact ZIP SHA256:

ebaa70eb3e781617b74e783932b15678029955a15deb1d8dedea0da9f67cb3b0


Artifact active main SHA256:

1adc4dfe948d616cbb7c6d9b3672842aba8e7d86bf0763e668505dff84234de0


Artifact subsdk9 SHA256:

96e45ba5175ccec696afee38ebcb4b454efdbd5c9945cfd79e506d5d15d54055


Artifact restore main SHA256:

2579b0cb85b79d5515a2518caeb5d5721168dbc1ec3f92c46eb372d13488ecd9


Artifact README SHA256:

77bde1190a780b7717f9460a44b69a2503d1bcb9e79728e191fcb150a2514819


Artifact manifest hashes matched computed files.
...
P83 startup contract:

baseline_p82=1

slot1928=0x7D34E0

caller_return=0x7F474C
...
----------------------------------------------------------------------
98.3 P83A DECISIVE RUNTIME CHAIN
----------------------------------------------------------------------
...
P82 0x7D2DE4 was reached at caller return 0x7F4780

then native state/action route produced:

requested=700

PlayAction700

ACTION_ROUTE route=UJ
...
The slot +0x1928 decision is a real gate capable of opening the native
Ultimate-Jutsu route.

P83 did NOT need direct force700 or direct state87 injection.


----------------------------------------------------------------------
98.4 CRITICAL P83 REGRESSION
----------------------------------------------------------------------

USER-OBSERVED REGRESSION:

Pressing ordinary XA also launches Ultimate Jutsu.
...
XA can also satisfy the current P83 compatibility policy and enter the
native UJ route.


Therefore:

P83A IS NOT A FINAL FIX.


The existing P83 predicates:

generated OugiAwakening membership
+
semantic UJ state
+
exact slot1928 caller
+
native result zero

are not sufficient to distinguish player input intent.


----------------------------------------------------------------------
98.5 CORRECTED INTERPRETATION
----------------------------------------------------------------------

PROVEN:

slot +0x1928 is a meaningful native UJ gate.

PROVEN:

overriding its native zero can produce the real native 700 route.

PROVEN:

semantic state can be active in a context that is still ordinary XA
from the player's input-intent perspective.

Therefore:

semantic=1 MUST NOT be treated as equivalent to:

"the player intentionally requested Ultimate Jutsu."


The missing piece is now:

INPUT / ROUTER INTENT DISCRIMINATION.


This is a narrower problem than the original UJ handoff failure.


======================================================================
99. R84 — P84A INTENT DISCRIMINATOR SOURCE READY
======================================================================

P84A replaces P83A as the directly active experiment.
...
Direct P83 installer is NOT active in main.


Observed P84 source markers:
...
P84A_SOURCE_PATCH=PASS

P84A_SOURCE_SANITY=PASS
...
P84A exists to distinguish:

ordinary XA / normal jutsu intent

from:

XXA / Ultimate-Jutsu intent


before any slot1928 compatibility override is allowed again.


P84A is currently an OBSERVATION / DISCRIMINATION phase.

It is NOT yet classified as a final gameplay fix.


----------------------------------------------------------------------
99.2 SLOT1928 POLICY STATUS
----------------------------------------------------------------------

P83's unconditional semantic+membership compatibility override must not
be treated as final behavior.

P84 must observe the native slot1928 route separately for XA and XXA.
...
is intended to provide runtime evidence around this boundary.


Do NOT restore the P83 native-zero -> policy-one behavior until an
independent intent discriminator is runtime-proven.
...
This checkpoint records only the existence of that instrumentation.

Its exact semantic meaning is NOT YET PROVEN RUNTIME.

Do not name it as a definitive:

UJ button detector,
combo detector,
input latch,
or final intent flag

until XA-vs-XXA runtime comparison proves that interpretation.


Current classification:

SOURCE / INSTRUMENTATION PRESENT
...
======================================================================
100. R84 NEXT RUNTIME — XA vs XXA DISCRIMINATION
======================================================================

Tobi begins the match already awakened.

Do NOT perform an awakening activation step.


----------------------------------------------------------------------
100.1 TEST A — ORDINARY XA
----------------------------------------------------------------------

From neutral battle state:
...
Capture the complete log around the attempt.
...
P57 requested action

P59 PlayAction

any P81 / P77 / P82 events that still remain in the installed baseline


The key question:

What value / call sequence exists during XA?


----------------------------------------------------------------------
100.2 TEST B — XXA / UJ INTENT
----------------------------------------------------------------------

Return to neutral state.

Perform the intended XXA Ultimate-Jutsu input.
...
What runtime fact exists for XXA but NOT XA?


----------------------------------------------------------------------
100.3 MINIMUM EVIDENCE STANDARD
----------------------------------------------------------------------

Do not build a gameplay policy from a single ambiguous event.

Preferred evidence:

at least two clean XA attempts

and

at least two clean XXA attempts.


A candidate discriminator is acceptable only if it is repeatable and
separates both input classes.


======================================================================
101. HARD REMINDERS / DO-NOT-REOPEN LIST
======================================================================

1. Tobi starts already awakened.

Never require an awakening-button step before UJ testing.


2. Event236 safety remains mandatory.

P50 shadow handling for custom Event236 operations must remain.

Do NOT restore raw native Event236 behavior that previously caused the
custom character to disappear / become unplayable after enemy UJ.


3. No gameplay branch:
...
Current ID 281 may exist only in generated source-derived membership
data / audited verifier fixture.


4. Keep OugiAwakening membership data-driven.

Current audited source payload SHA256:

4322e4ab30547321abc6cd24322a5ba98f8e7d0ddafa853039234c397ccfe6cb


Current audited fixture membership:

281


The final architecture must continue to support additional mod
characters without adding new hardcoded behavior branches.
...
6. Do NOT inline-hook the previously rejected hot virtual BLR sites.

In particular do not reopen blind inline replacement at:

0x7F4588

or other previously regressive BLR callsites.


7. Do NOT mutate SW_MTOB_XH merely to bypass the earlier route.

Do NOT delete the condition.

Do NOT rewrite descriptor field +0x0C merely because its current value
is 1.


8. P77 naming correction remains active.

The historical log label:

[NSC:P77A] UJ_ACCEPT

does NOT mean final UJ acceptance.

ret=1 only means that helper predicate is positive.


9. P81 remains an important proven result.

P81 proved the +0x1288 awakened/type1 veto and cleared it using
data-driven Ougi membership plus semantic context.

Do not reopen the old condition512 investigation as the current root
unless new runtime evidence requires it.


10. P83 remains diagnostic proof, not final policy.

P83 proved slot1928 is a real route to native UJ.

P83 also proved that semantic+membership alone is too broad for final
input gating.


11. Do NOT transplant PC RVAs directly to Switch.

PC behavior/source may guide semantics only after an independent Switch
runtime/static homolog is proven.


12. The old 0x7E1404 / 0x7E1408 tail modification is not the current
root and must not be reopened.


13. main+0x7E0F58 historical ABI remains:

void(actor, uint32_t mode)

Do not retype it as bool.


14. No persistent generic getter override.

Do not globally replace native control-state getters merely to make UJ
pass.


15. Preserve paired-main identity.

Active paired main SHA256:

1adc4dfe948d616cbb7c6d9b3672842aba8e7d86bf0763e668505dff84234de0


Vanilla / restore main SHA256:
...
remain the authoritative project checkpoint copies.


Before declaring any checkpoint confirmed:
...
downloaded build ZIPs

temporary extracted artifacts


Historical probe workflows should remain manual-only.

Only the current experiment workflow should be automatic.


prepare_exlaunch_p67a.sh must continue copying:
...
A historical verifier failure caused by a newer active main is not a
gameplay failure.


======================================================================
103. CURRENT DECISION FRONTIER
======================================================================

OLD QUESTION:

Why does Tobi fail to enter native UJ?


ANSWER:

P81 and P83 together proved enough native gates to reach the real
action700 route.


CURRENT QUESTION:

How does the native router distinguish ordinary XA from intentional
XXA / Ultimate-Jutsu input?


P84A exists specifically to answer this.


NEXT ACTION:

BUILD P84A ONLY AFTER STRICT STATIC / SOURCE VERIFICATION.

Then run controlled XA-vs-XXA tests.

Do not introduce another gameplay override until the discriminator is
proven.



END MASTER CHECKPOINT R84
