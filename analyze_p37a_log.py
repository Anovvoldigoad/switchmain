...
rows=[x for x in lines if '[NSC:P37A]' in x]
print('P37A markers=',len(rows))
for key in ('READY','VIS_SHADOW','STAGE_HANDLE','FIX_CHAR','POST_STAGE','EVT235_SHOW','EVT236','FILE_OPEN','PROCESS'):
    print(f'{key}=',sum(f'[NSC:P37A] {key}' in x for x in rows))
