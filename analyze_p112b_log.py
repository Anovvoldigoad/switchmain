...
if len(sys.argv)!=2:
    print('usage: python3 analyze_p112b_log.py uzuy_log.txt'); raise SystemExit(2)
lines=Path(sys.argv[1]).read_text(errors='replace').splitlines()
ready=[(i+1,l) for i,l in enumerate(lines) if '[NSC:P112B] READY' in l]
rx=re.compile(r'\[NSC:P112B\] ENTRY n=(\d+) actor=(0x[0-9a-f]+) side=(\d+) char=(\d+) caller_off=(0x[0-9a-f]+) '
