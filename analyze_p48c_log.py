...
PATTERNS = {
    'play': re.compile(r'PLAY_ACTION actor=(0x[0-9a-fA-F]+|\(nil\)) valid=(\d+) side=(\d+) char=(\d+) index=(-?\d+) ret=(-?\d+) n=(\d+).*?pre_action=(\d+).*?post_action=(\d+)'),
...
def main(path):
    ...
    for line in lines:
        if '[NSC:P48C]' not in line: continue
