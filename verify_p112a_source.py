...
start=cpp.find('// P112A — exact state125 producer entry provenance')
assert start>=0
block=cpp[start:]
hs=cpp.find('HOOK_DEFINE_TRAMPOLINE(P112State125ProducerEntryHook)',start)
he=cpp.find('static bool InstallP112State125ProducerEntryInternal()',hs)
...
checks={
 'entrypoint':'nsc::InstallP112State125ProducerEntryProbe();' in main,
 'decl':'void InstallP112State125ProducerEntryProbe();' in hpp,
 'parent_p96':'InstallP96AActionDescriptorTransitionTrace();' in block,
 'ready':'[NSC:P112A] READY' in block,
 'entry_log':'[NSC:P112A] ENTRY' in block,
...
def bounds(src):
 ...
 for m in re.finditer(r'Logging\.Log\((.*?)\);',src,re.S):
  ...
  if '[NSC:P112A]' not in fmt: continue
  total=0; last=0
...
print('push_enabled_workflows',push); assert push==['build-subsdk9-p112a.yml']
print('P112A_DROPIN_SOURCE_VERIFY=PASS')
